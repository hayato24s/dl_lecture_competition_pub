...
compapred to 004, trainval split is [0.9, 0.1].

qrsh -g $GROUP -l rt_G.large=1 -l h_rt=1:00:00
tmux new -s s0
source venv/bin/activate
python main_008.py train

qrsh -g $GROUP -l rt_G.small=1 -l h_rt=1:00:00
tmux new -s s0
source venv/bin/activate
python main_008.py predict
"""
...
def main_train():
    ...
    output_dir_path = Path("outputs/008")
    ...
    train_dataset, valid_dataset = torch.utils.data.random_split(
        train_valid_dataset, lengths=[0.9, 0.1]
    )
...
def main_predict():
    ...
    litmodule = LitModule.load_from_checkpoint(
        checkpoint_path="outputs/008/checkpoints/epoch=027-val_vqa_metric=0.5916.ckpt",
        map_location="cpu",
        model=model,
    )
